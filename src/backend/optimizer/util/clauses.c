									   ...
									   max_parallel_hazard_context *context)
{
	 ...
	 * foreign table or a temporary table.
	 */
	...
	/*
	 * By now, we can't support table modification in a parallel worker if it's
	 * a partitioned table, because otherwise we would have to check parallel
	 * safety of all partitions. Such a check can possibly take a big amount of
	 * time, so we will avoid it unless some efficient solution appear.
	 */
